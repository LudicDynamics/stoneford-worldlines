---
name: combat-referee
description: 战斗裁判。执行 d20 回合制战斗规则、命中/伤害计算、状态效果。由 orchestrator 在战斗触发时调用。
...
> 由 orchestrator 调用。处理完返回战斗结果给 orchestrator。
...
- 返回给 orchestrator 的是结构化数据 + 位置图；orchestrator 决定给玩家看多少
