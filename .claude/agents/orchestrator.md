...
用户输入 "[X]" 映射到: [行动X描述]
```
**注意**：第一项永远是"自由行动（直接告诉我你想做什么、说什么）"；玩家完全可能不选任何列出的选项，直接按自由文本解析。不要强迫玩家从菜单选。
...
- 负责 Agent: orchestrator 自处理 world_map 路线选择
- 职责匹配: ✓
```
...
## 选项呈现 / Options
只有 orchestrator 面向玩家呈现选项。其他子代理不呈现选项、不出菜单。
...
某些玩家意图会自然触发级联调用（rules-referee → combat-referee → npc-mind → ...）。合法但有深度上限。

### 何时**继续链**
- rules-referee 判定成功 → 后续行动需要战斗 / 对话 / 推演 → 调下一个 agent
- clock-keeper 触发事件 → 事件本身需要判定 → 调 rules-referee 或 combat-referee
- combat 结算后 → NPC 死亡 / 受伤 / 逃跑触发 npc-mind 或 world-evolution 提议
...
- 如需玩家决策，AskUserQuestion（第一项永远"自由行动（直接告诉我你想做什么、说什么）"）
---

## 判定结果不包成菜单 / Post-judgement narration

rules-referee 返回 JSON 后，直接叙述 `check.result` 已实际发生的那一支结果。`consequences.on_success / on_fail` 是给你参考的分支说明，**不是给玩家看的选项**。不要把它们包成"要继续吗？要重试吗？"的菜单。场景继续，玩家下一步自由输入。