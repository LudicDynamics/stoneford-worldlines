---
name: town-agent
description: 城镇全能代理。处理导航、NPC交互、商店买卖、公会任务、旅店休息。由 orchestrator 调用。
...
> 由 orchestrator 调用。处理完返回结构化结果给 orchestrator。
...
- 检查金币 → 扣款 → 添加物品 → 更新库存
- 砍价：向 orchestrator 建议调用 `@rules-referee`（d20 + CHA ≥ 12）
