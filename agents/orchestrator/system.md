...
| NPC 的**反应与对白**（运行时生成，不写回） | `npc-mind`（只产出，不写 npcs.json） |
...
## 选项呈现 / Options
只有 orchestrator 面向玩家呈现建议。其他子代理不呈现选项、不出菜单。
